...
- (void)viewDidLoad {
    ...
    if(self.taskArray.count == 0){
        [self saveToCoreData:@"Walk the dog"];
        [self.taskArray addObject:@"Walk the dog"];
        [self saveToCoreData:@"Feed the hogs"];
        [self.taskArray addObject:@"Feed the hogs"];
        [self saveToCoreData:@"Chop the logs"];
        [self.taskArray addObject:@"Chop the logs"];
...
- (IBAction)addTaskButtonClick:(id)sender {
    ...
    if ([inputStr isEqual: @""]) {
        //弹出提示
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"错误" message:@"输入的内容不能为空" preferredStyle:UIAlertControllerStyleAlert];
